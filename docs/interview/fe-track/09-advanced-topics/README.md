...
> Source scope: all `.md` files under `docs/interview/fe-track/09-advanced-topics/` (excluding this README).
...
They exist to keep FE-track navigation complete, but the canonical theory source of truth now lives under `docs/interview/shared/`.

Chúng tồn tại để giữ điều hướng FE-track đầy đủ, nhưng nguồn lý thuyết chuẩn (single source of truth) hiện nằm trong `docs/interview/shared/`.
