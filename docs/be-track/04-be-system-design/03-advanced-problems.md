...
| #   | Problem                  | Vai Trò Trong Interview                                                        | Interview Weight |
| --- | ------------------------ | ------------------------------------------------------------------------------ | ---------------- |
| 1   | Notification System      | Multi-channel delivery, priority queue, retry/fallback, multi-tenant isolation | ⭐⭐⭐           |
| 2   | Rate Limiter             | Token bucket vs sliding window, distributed state, fail-open/closed strategy   | ⭐⭐⭐           |
| 3   | Payment System           | Idempotency, double-entry ledger, state machine, PSP reconciliation            | ⭐⭐⭐           |
| 4   | Ride-Matching / Location | Geo indexing (geohash/H3), real-time dispatch, surge pricing, ETA              | ⭐⭐⭐           |
| 5   | News Feed / Timeline     | Push vs pull fanout, celebrity problem, ranking pipeline, cache tiers          | ⭐⭐⭐           |
| 6   | Distributed File Storage | Chunking, replication vs erasure coding, metadata consensus, signed URL        | ⭐⭐             |

> 6 bài toán nâng cao bao phủ các mẫu thiết kế phổ biến nhất trong interview Senior: event-driven processing (Notification), distributed state management (Rate Limiter), financial correctness (Payment), spatial computing (Ride-Matching), read-heavy scale (News Feed), và storage durability (File Storage). Mỗi bài kết hợp FR/NFR estimation → HLD → data model → API design → deep dive → tradeoff analysis — đúng cấu trúc interview loop 45-60 phút. Các bài liên kết chặt qua shared patterns: idempotency (Payment ↔ Notification), distributed locking (Ride ↔ Rate Limiter), fanout (Feed ↔ Notification), consistency models (Payment ↔ Storage).
...
### Concept 1: Notification System / Hệ Thống Thông Báo

**🧠 Memory Hook:** "Bưu điện thông minh" — nhận thư, xem ưu tiên, chọn kênh (xe máy/thư/điện thoại), retry nếu không giao được.

**Tại sao tồn tại / Why exists:**

- **Level 1:** User cần biết ngay khi có OTP, đơn hàng thay đổi, hoặc khuyến mãi.
- **Level 2:** Gửi trực tiếp từ business service → coupling chặt, không scale, không retry, không audit.
- **Level 3:** Platform notification cho phép tách ingestion/routing/delivery, multi-tenant, priority isolation, và compliance (unsubscribe/DND/anti-spam).

**Layer 1 — Analogy / Tương tự đơn giản:**

> Notification system giống hệ thống bưu chính quốc gia: bạn gửi thư (event) → bưu điện (ingestion) → phân loại ưu tiên (routing) → chọn phương tiện giao (channel worker) → xác nhận đã giao (delivery status). Thư quan trọng (OTP) gửi express, quảng cáo gửi thường.

**Layer 2 — Mechanics / Cơ chế kỹ thuật:**

```text
Business Event → Ingestion API (validate + dedupe)
       │
       ▼
  Kafka Topics (notif.requested / notif.retry / notif.dlq)
       │
       ├─→ Preference Service (opt-in/out, quiet hours)
       ├─→ Template Service (render theo locale)
       │
       ▼
  Fanout + Routing Core (priority weighted 70/20/8/2, dedupe, rate limit)
       │
       ...
       │
       ▼
  Delivery Status Collector (webhook) → OLTP + Analytics
```

- Partition key = `user_id` để giữ ordering per user.
- Priority queue 4 cấp, worker dùng weighted scheduling, aging chống starvation.
- Idempotency key = `SETNX(idem:{key})` TTL 24h trước enqueue.

**Layer 3 — Edge cases / Trường hợp biên:**

- Provider outage toàn phần → circuit breaker + fallback channel (push fail → SMS).
- Marketing campaign tranh queue với transactional → tách topic/pool riêng.
- Multi-tenant blast radius: per-tenant DLQ + circuit breaker + quota.
- Exactly-once delivery bất khả thi với external channel → at-least-once + dedupe key.

| Sai lầm                                   | Tại sao sai                           | Đúng là                                    |
| ----------------------------------------- | ------------------------------------- | ------------------------------------------ |
| Gộp transactional + marketing vào 1 queue | Marketing burst làm trễ OTP           | Tách queue/topic theo priority class       |
| Không có idempotency key                  | Client retry → gửi trùng notification | SETNX dedupe trước enqueue                 |
| Retry không phân loại error               | Retry invalid email vĩnh viễn         | Error taxonomy: retryable vs non-retryable |
| Không có tenant quota                     | 1 tenant chiếm toàn bộ throughput     | Per-tenant rate limit + quota              |

**🎯 Interview Pattern:** "Hãy thiết kế notification system" → bắt đầu bằng tách transactional/marketing path → show priority queue + retry/DLQ → nhấn idempotency + multi-tenant isolation → chốt observability matrix.

**🔗 Knowledge Chain:** API Design (endpoint contract) → Message Queues (Kafka partition) → Resilience Patterns (circuit breaker, retry) → Observability (delivery funnel metrics)

---

### Concept 2: Rate Limiter / Bộ Giới Hạn Tốc Độ

**🧠 Memory Hook:** "Bảo vệ cổng thành" — đếm người vào, từ chối khi quá tải, VIP được burst, cổng hỏng thì mở hay đóng?

**Tại sao tồn tại / Why exists:**

- **Level 1:** Bảo vệ hệ thống khỏi bị overwhelm bởi traffic spike hoặc abuse.
- **Level 2:** Không có rate limit → 1 user/bot có thể DoS toàn hệ thống, hoặc 1 tenant chiếm hết resource.
- **Level 3:** Distributed rate limiting đảm bảo fairness across instances, cho burst hợp lý, và có policy khác nhau theo endpoint/tenant/user.

**Layer 1 — Analogy / Tương tự đơn giản:**

> Rate limiter giống cổng soát vé sân vận động: mỗi phút chỉ cho 100 người qua (fixed window), hoặc mỗi người có "thẻ token" được nạp đều đặn (token bucket). VIP có thẻ dung lượng lớn hơn (burst). Khi máy soát hỏng: mở cổng tự do (fail-open) hay đóng hết (fail-closed)?
...
┌──────────────────┬───────────────┬──────────────┬───────────────┐
│ Fixed Window     │ Sliding Log   │ Sliding Ctr  │ Token Bucket  │
├──────────────────┼───────────────┼──────────────┼───────────────┤
│ count per window │ store each ts │ sub-window   │ refill tokens │
│ boundary burst!  │ precise       │ approximate  │ burst allowed │
│ O(1) memory      │ O(N) memory   │ O(sub-wins)  │ O(1) memory   │
└──────────────────┴───────────────┴──────────────┴───────────────┘

Architecture: Gateway → gRPC → Rate Limit Service → Redis Cluster (atomic Lua)
```

- Redis Lua script cho token bucket: atomic HMGET/HMSET + PEXPIRE.
- Consistent hashing route cùng subject về cùng Redis shard.
- Multi-region: local hard limit + global soft quota (token leasing).
- Multi-dimensional: check tenant → user → endpoint → ALL phải pass.

**Layer 3 — Edge cases / Trường hợp biên:**

- Fixed window boundary burst: 200 req trong 2s tại ranh giới window.
- Hot key (1 subject cực nóng) → local leaky bucket trước remote check.
- Redis outage → fail-open cho read/public, fail-closed cho auth/payment.
- Global multi-region sync → token leasing (central allocator cấp batch).

| Sai lầm                              | Tại sao sai                           | Đúng là                                            |
| ------------------------------------ | ------------------------------------- | -------------------------------------------------- |
| Dùng fixed window cho mọi trường hợp | Boundary burst gấp đôi limit          | Sliding window hoặc token bucket                   |
| Fail-open mọi endpoint               | Payment/auth bị abuse khi Redis down  | Fail-closed cho critical endpoint                  |
| Chỉ rate limit ở 1 chiều             | 1 user spam dưới tenant limit vẫn qua | Multi-dimensional quota (tenant + user + endpoint) |
| Gọi Redis cross-region mỗi request   | Latency 50-100ms mỗi request          | Local limit + global soft quota async              |

**🎯 Interview Pattern:** "Design a rate limiter" → clarify scope (per-user? per-tenant? global?) → compare algorithms → show Redis atomic Lua → discuss fail-open/closed strategy → chốt multi-region design.

**🔗 Knowledge Chain:** Networking (HTTP 429 headers) → Redis (atomic operations, Lua) → Distributed Systems (consistency, partitioning) → Resilience Patterns (circuit breaker cho Redis)

---

### Concept 3: Payment System / Hệ Thống Thanh Toán

**🧠 Memory Hook:** "Két sắt kép" — mỗi giao dịch ghi 2 dòng sổ (debit + credit), idempotency key như mã vạch chống scan trùng, reconciliation như kiểm kê cuối ngày.

**Tại sao tồn tại / Why exists:**

- **Level 1:** User cần thanh toán an toàn, không bị trừ tiền 2 lần.
- **Level 2:** PSP callback async + out-of-order → cần state machine + idempotency để đảm bảo exactly-once business effect.
- **Level 3:** Double-entry ledger đảm bảo DEBIT = CREDIT mọi lúc, reconciliation T+1 phát hiện mismatch, PCI compliance minimize scope.

**Layer 1 — Analogy / Tương tự đơn giản:**

> Payment system giống ngân hàng thu ngân: khách đưa thẻ (intent) → kiểm tra số dư (authorize) → giữ tiền tạm (hold) → xác nhận giao dịch (capture) → ghi sổ kép (ledger). Cuối ngày đối chiếu sổ với ngân hàng đối tác (reconciliation). Mỗi phiếu giao dịch có mã vạch unique (idempotency key) — scan 2 lần vẫn chỉ tính 1.

**Layer 2 — Mechanics / Cơ chế kỹ thuật:**

```text
State Machine:
  created ──(auth)──→ authorized ──(capture)──→ captured ──(refund)──→ refunded
     │                    │
...
Double-Entry Ledger:
  [Capture $100]
  DEBIT  buyer_clearing    $100
  CREDIT merchant_payable  $100
  ∑ = 0 ✓ (invariant)

Idempotency: lock(key) → check cache → process → persist response → unlock
```

- SELECT ... FOR UPDATE trên payment intent chống double spending.
- Webhook signature verification bắt buộc, apply state theo precedence + version.
- Reconciliation T+1: normalize PSP file → match (psp_txn_id, amount, date) → classify mismatch.

**Layer 3 — Edge cases / Trường hợp biên:**

- Webhook đến trễ 2 giờ sau capture → idempotent update, không duplicate entry.
- PSP trả timeout nhưng thực ra đã authorize → reconciliation phát hiện + manual review.
- Partial refund rồi full refund → state machine reject nếu tổng refund > captured amount.
- Multi-PSP failover: orchestrator retry sang PSP backup khi primary timeout.

| Sai lầm                      | Tại sao sai                             | Đúng là                                      |
| ---------------------------- | --------------------------------------- | -------------------------------------------- |
| Không dùng idempotency key   | Client retry → trừ tiền 2 lần           | Lock + cache response + TTL > retry window   |
| Lưu PAN/CVV trong DB chính   | Vi phạm PCI DSS, rủi ro data breach     | Tokenization từ PSP, hosted fields           |
| Không có state machine guard | Capture 2 lần trên cùng intent          | Validate transition hợp lệ trước khi proceed |
| Bỏ qua reconciliation        | Mismatch tích lũy → mất tiền không biết | T+1 batch đối soát + auto-heal + alert       |

**🎯 Interview Pattern:** "Design a payment system" → emphasize idempotency first → show state machine + double-entry ledger → discuss PSP integration + webhook handling → chốt reconciliation workflow + PCI scope.

**🔗 Knowledge Chain:** API Design (idempotent endpoints) → Distributed Systems (exactly-once semantics) → Database (ACID, row locking) → Security (PCI DSS, tokenization)

---

### Concept 4: Ride-Matching / Real-Time Location

**🧠 Memory Hook:** "GPS radar + auction" — 1.5M xe online gửi tọa độ mỗi 3s, rider tạo booking → radar quét bán kính → đấu giá theo ETA + fairness → reserve atomic.

**Tại sao tồn tại / Why exists:**

- **Level 1:** Rider cần tìm driver gần nhất nhanh nhất.
- **Level 2:** Naive approach (scan all drivers) → O(N) mỗi booking, không scale với 1.5M drivers.
- **Level 3:** Geo indexing (geohash/H3/S2) cho O(1) spatial lookup, matching pipeline score ETA + reliability + fairness, surge pricing balance cung-cầu.

**Layer 1 — Analogy / Tương tự đơn giản:**

> Ride-matching giống đài điều phối taxi: bản đồ chia thành ô vuông (geohash), mỗi ô biết có bao nhiêu xe trống. Khách gọi → tìm ô gần → lọc xe phù hợp → chọn xe tốt nhất (ETA + xếp hạng) → gửi offer → nếu từ chối thì mở rộng phạm vi. Giờ cao điểm/mưa → giá surge để kéo thêm tài xế online.

**Layer 2 — Mechanics / Cơ chế kỹ thuật:**

```text
Matching Pipeline:
  1. Geo query: geohash prefix 1-2km → candidate list
  2. Filter: vehicle type, status=available, acceptance score
  3. Score: w1*ETA + w2*accept_prob + w3*fairness_term
  4. Offer batch (top 3), timeout 8-10s
  5. Fail → expand radius → retry wave 2

Location Ingestion: 450K updates/sec
  Driver App → Location Gateway → Stream Processor → Geo Index (Redis/H3)

Atomic Reserve: Redis SETNX "drv:reserve:{id}" {requestId} PX {ttl}
```

- Geo index options: Geohash (simple, boundary issues), H3/S2 (uniform cells), Quadtree (recursive split).
- ETA: Haversine quá đơn giản → road graph + real-time traffic speed profile.
- Surge: per cell (H3), 5-min window, `surge = f(demand/supply, weather)`, capped + smoothed.

**Layer 3 — Edge cases / Trường hợp biên:**

- Driver nhận 2 booking cùng lúc → atomic compare-and-set (SETNX + TTL).
- Geohash boundary: candidate ở cell lân cận bị miss → query neighboring cells.
- GPS jitter trong tunnel → dead-reckoning smoothing + discard out-of-order.
- City-scale event (concert, mưa): surge clamp + fairness rebalance + capacity mode.

| Sai lầm                               | Tại sao sai                                | Đúng là                                         |
| ------------------------------------- | ------------------------------------------ | ----------------------------------------------- |
| Chỉ dùng Haversine distance cho ETA   | Đường thực tế khác đường chim bay          | Road graph + traffic speed profile              |
| Score chỉ theo ETA                    | Tài xế hotspot nhận hết cuốc, ngoại vi đói | Fairness term: idle time bonus + recent penalty |
| Không query neighboring geohash cells | Miss driver ở biên cell                    | Query cell + 8 neighbors cùng precision         |
| Surge không có cap/smoothing          | Giá nhảy loạn, user churn                  | Hysteresis + multiplier clamp + decay           |

**🎯 Interview Pattern:** "Design Grab/Uber" → start với location ingestion scale (450K/s) → show geo index choice → matching pipeline 5 bước → atomic reserve → surge pricing → chốt consistency + failure mode.

**🔗 Knowledge Chain:** Database (spatial indexing, Redis GEO) → Distributed Systems (consistency, partitioning) → Algorithms (graph shortest path, scoring) → Resilience (dispatch retry, capacity planning)

---

### Concept 5: News Feed / Timeline

**🧠 Memory Hook:** "Bưu tá vs thư viện" — fanout-on-write = bưu tá mang báo đến từng nhà (nhanh đọc, chậm ghi), fanout-on-read = ra thư viện tự tìm (chậm đọc, nhanh ghi). Celebrity = nhà xuất bản triệu bản → dùng hybrid.

**Tại sao tồn tại / Why exists:**

- **Level 1:** User muốn xem bài mới từ người mình follow, cá nhân hóa, real-time.
- **Level 2:** Naive approach join follow graph mỗi request → O(follows × posts) per read, quá chậm với 300K reads/s.
- **Level 3:** Materialized inbox (push model) cho read nhanh, nhưng write explosion cho celebrity. Hybrid push/pull + ranking pipeline giải quyết cả hai.

**Layer 1 — Analogy / Tương tự đơn giản:**

> News feed giống hệ thống phát báo: bạn đăng ký (follow) các tòa soạn. Push model = tòa soạn in báo giao tận nhà mỗi sáng (inbox materialized). Pull model = bạn ra sạp báo chọn từng tờ (query on read). Celebrity = tòa soạn 100M độc giả → không thể in 100M bản mỗi bài → chuyển sang "đặt ở sạp, ai cần thì lấy" (pull + cache).

**Layer 2 — Mechanics / Cơ chế kỹ thuật:**

```text
Write Path:
  Post API → Post Store (metadata + content)
       │
       └→ Fanout Service ─→ user_inbox table (post_id, score)
              │
              ├─ Normal user: push to all followers' inbox
              └─ Celebrity: skip push, mark for pull

Read Path:
  Feed Query → merge(inbox_cache, pull_celebrity_posts) → Ranking Service
       │
       ├─ Candidate retrieval (10K items)
       ├─ Lightweight scoring (→ 500)
       ├─ Heavy model ranking (→ 100)
       ├─ Re-rank diversity/safety
       └─ Final paging (→ 20)
```

- Cursor-based pagination tránh offset scan.
- First page cache per user, short TTL (10-30s), stale-while-revalidate.
- Follow graph stored in social graph service, partitioned by user_id.

**Layer 3 — Edge cases / Trường hợp biên:**

- Celebrity 100M followers post → chỉ pull + precompute top K + cache warm.
- Block/mute/privacy change → near-real-time invalidation, không chờ TTL expire.
- Feed ranking dependency timeout → fallback chronological ranking + cache last-good features.
- Write amplification monitoring: track fanout ratio per post, alert khi bất thường.

| Sai lầm                                    | Tại sao sai                                | Đúng là                                     |
| ------------------------------------------ | ------------------------------------------ | ------------------------------------------- |
| Push model cho tất cả user kể cả celebrity | Celebrity 100M followers → write explosion | Hybrid: push cho normal, pull cho celebrity |
| Offset-based pagination                    | Page 1000 → scan 20K rows                  | Cursor-based (score + ts + post_id)         |
| Cache TTL dài cho feed                     | User không thấy post mới                   | Short TTL + event-driven invalidation       |
| Ranking chỉ theo chronological             | Engagement thấp, user bỏ app               | ML ranking + diversity + safety re-rank     |

**🎯 Interview Pattern:** "Design news feed" → clarify scale (80M DAU, 300K reads/s) → show push/pull tradeoff → propose hybrid → ranking pipeline → chốt celebrity problem + cache strategy.

**🔗 Knowledge Chain:** Caching Patterns (invalidation, stale-while-revalidate) → Message Queues (fanout) → Database (social graph, cursor pagination) → ML Systems (ranking pipeline)

---

### Concept 6: Distributed File Storage / Lưu Trữ File Phân Tán

**🧠 Memory Hook:** "Tủ hồ sơ phân tán" — file lớn cắt thành chunk (ngăn kéo), mỗi ngăn kéo có 3 bản copy ở 3 tòa nhà khác nhau (replication), hoặc mã hóa toán học (erasure coding) để tiết kiệm. Sổ mục lục (metadata) dùng consensus để không bao giờ sai.

**Tại sao tồn tại / Why exists:**

- **Level 1:** User cần upload/download file lớn (MB-GB) với durability cao.
- **Level 2:** Single server → capacity giới hạn, SPOF, không resumable. Cần phân tán data + metadata.
- **Level 3:** 11 nines durability cần replication/erasure coding cross-AZ, metadata consensus (Raft), scrub + repair pipeline, lifecycle management (hot → warm → cold → archive).

**Layer 1 — Analogy / Tương tự đơn giản:**

> File storage giống kho hàng quốc tế: hàng lớn (file) chia thành kiện nhỏ (chunk 8-64MB) → mỗi kiện gắn barcode (hash) → gửi 3 bản đến 3 kho khác nhau (replication 3x). Sổ quản lý kho (metadata) dùng 3 thủ kho vote (Raft consensus) để tránh ghi sai. Hàng cũ chuyển kho giá rẻ hơn (tiering).

**Layer 2 — Mechanics / Cơ chế kỹ thuật:**

```text
Upload Flow:
  Client → Metadata API (create upload session)
       │
       ├─ Chunk Placement Service (chọn 3 storage nodes cross-AZ)
       │
       ├─ PUT chunk to Storage Nodes (parallel)
       │        ├─ Node AZ-1 (primary)
       │        ├─ Node AZ-2 (replica)
       │        └─ Node AZ-3 (replica)
       │
       └─ Complete upload → Metadata API (commit manifest)
            └─ Quorum write 2/3, verify all chunk hashes

Download Flow:
  Client → Metadata API (resolve manifest) → parallel GET chunks
```

- Erasure coding (10+4): 40% overhead vs 200% cho 3x replication, nhưng encode/decode CPU cao.
- Metadata partition theo `(bucket, key_prefix_hash)`, secondary index cho prefix listing.
- Signed URL: TTL ngắn + bind method + nonce chống replay.

**Layer 3 — Edge cases / Trường hợp biên:**

- Node chết giữa upload → client retry part sang node khác (resumable).
- Split brain metadata → Raft consensus cho control plane.
- Bit rot (silent corruption) → scrub job định kỳ + repair từ replica/parity.
- Read-after-write consistency cho object mới → strong consistency cho metadata create.

| Sai lầm                            | Tại sao sai                                 | Đúng là                                       |
| ---------------------------------- | ------------------------------------------- | --------------------------------------------- |
| Chunk quá nhỏ (1MB)                | Metadata overhead + request overhead cao    | 8-64MB tùy network profile                    |
| 3x replication cho mọi data        | Hot data OK, cold data quá tốn              | Hybrid: replication hot, erasure coding cold  |
| Metadata dùng eventual consistency | Object vừa upload nhưng list không thấy     | Strong consistency cho metadata create/delete |
| Không có scrub job                 | Bit rot tích lũy, phát hiện khi đã quá muộn | Periodic scrub + repair pipeline              |

**🎯 Interview Pattern:** "Design S3/GFS" → start với upload flow (chunking + placement) → replication strategy → metadata HA (Raft) → show durability math (11 nines) → chốt lifecycle tiering + signed URL.

**🔗 Knowledge Chain:** Distributed Systems (replication, consensus, partitioning) → Database (metadata indexing) → Networking (multipart upload, signed URL) → OS (file system, I/O models)
...
## Interview Q&A Summary / Tóm Tắt Câu Hỏi Phỏng Vấn

> **Tổng: ~115 câu hỏi** (8 🟢 Junior, 52 🟡 Mid, 55 🔴 Senior)

| Section                 | # Qs | 🟢  | 🟡  | 🔴  | Key Signals                                                        |
| ----------------------- | ---- | --- | --- | --- | ------------------------------------------------------------------ |
| P1: Notification System | 6    | 0   | 4   | 2   | Priority isolation, retry taxonomy, idempotency key                |
| P2: Rate Limiter        | 6    | 1   | 3   | 2   | Token bucket vs sliding window, fail-open/closed, Redis Lua atomic |
| P3: Payment System      | 6    | 0   | 4   | 2   | State machine guard, double-entry invariant, PCI scope             |
| P4: Ride-Matching       | 5    | 0   | 3   | 2   | Geohash + neighbor query, atomic reserve SETNX, surge smoothing    |
| P5: News Feed           | 5    | 0   | 3   | 2   | Hybrid push/pull, celebrity problem, cursor pagination             |
| P6: File Storage        | 5    | 0   | 3   | 2   | Chunk sizing, replication vs erasure coding, metadata Raft         |
| Interview Drill         | 6    | 1   | 2   | 3   | Cross-problem pattern recognition                                  |
| P1-P6 Extended Add-ons  | 30   | 0   | 12  | 18  | Multi-tenant, exactly-once, fairness, ranking pipeline             |
| Estimation Workbook     | 5    | 0   | 5   | 0   | Partition count, IOPS, storage/day, cache memory                   |
| Failure Mode Catalog    | 6    | 0   | 0   | 6   | Provider outage, Redis failure, recon mismatch, dispatch spike     |
| API Error Contracts     | 3    | 2   | 1   | 0   | Structured error JSON, 429 headers, idempotent replay              |
| Whiteboard Prompts      | 6    | 1   | 1   | 4   | 3-minute sketch, multi-PSP failover, 100M followers                |
| Mini Case Studies       | 5    | 0   | 5   | 0   | OTP delay, double charge, driver starvation, feed stale            |
| Go Snippets             | 4    | 0   | 4   | 0   | Sliding window, state machine, cursor encode, checksum             |
| Final Review Checklist  | 3    | 1   | 1   | 1   | FR/NFR, bottleneck, consistency model per path                     |
| Extra Q&A Pack          | 8    | 2   | 2   | 4   | Scope clarification, outbox pattern, DLQ ownership                 |
| Bilingual Deep Q&A      | 5    | 0   | 2   | 3   | Distributed locking, consistent hashing, CQRS, Event Sourcing      |

### Per-Problem Key Interview Signals

| Problem       | Strong Signal (Senior)                                                            | Weak Signal (Junior)             |
| ------------- | --------------------------------------------------------------------------------- | -------------------------------- |
| Notification  | Tách transactional/marketing queue + multi-tenant DLQ + error taxonomy            | Chỉ nói "dùng Kafka gửi message" |
| Rate Limiter  | Token bucket Lua atomic + multi-dimensional quota + fail-open/closed per endpoint | Chỉ biết fixed window counter    |
| Payment       | Idempotency key + state machine + double-entry + reconciliation T+1               | Chỉ nói "lưu vào DB"             |
| Ride-Matching | Geohash + neighbor expansion + atomic reserve + fairness scoring                  | Chỉ nói "tìm driver gần nhất"    |
| News Feed     | Hybrid fanout + celebrity exception + ranking pipeline + cursor pagination        | Chỉ nói "push bài vào timeline"  |
| File Storage  | Chunk + quorum write + erasure coding + Raft metadata + scrub                     | Chỉ nói "lưu file lên server"    |

---

## ⚡ Cold Call Simulation / Mô Phỏng Hỏi Bất Chợt

> **Tình huống:** "You need to design a payment system handling 8K TPS with multi-PSP support. How do you prevent double-charging?"

**Trả lời trong 30 giây:**

"Idempotency key scope là `(merchant_id, endpoint, key)` — lần đầu lock + process + persist response, lần retry trả response cũ. Payment intent dùng state machine với `SELECT ... FOR UPDATE` chống double transition. Double-entry ledger đảm bảo `∑DEBIT = ∑CREDIT` per transaction. Reconciliation T+1 match internal ledger với PSP settlement file phát hiện mismatch."

**Follow-up:** "PSP trả timeout nhưng thực ra đã authorize — xử lý sao?"
→ "Persist timeout event, retry với idempotency key (PSP cũng hỗ trợ), reconciliation T+1 sẽ phát hiện nếu authorize thành công. Không retry capture trên timeout authorize — chờ reconciliation confirm hoặc manual review."
...
| #   | Dạng            | Câu hỏi                                                                                                                                          |
| --- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | **Retrieval**   | Kể tên 6 bài toán advanced và cho mỗi bài 1 key pattern (ví dụ: Notification → priority queue isolation)                                         |
| 2   | **Visual**      | Vẽ payment state machine: created → authorized → captured → refunded. Có những transition nào bất hợp lệ?                                        |
| 3   | **Application** | Bạn đang thiết kế ride-matching cho 1.5M drivers online. Geohash query cho bán kính 2km, nhưng khu vực giáp biên cell bị miss driver. Giải pháp? |
| 4   | **Debug**       | Feed system đang dùng push model cho tất cả user kể cả celebrity 50M followers. Triệu chứng gì xảy ra? Fix như thế nào?                          |
| 5   | **Teach**       | Giải thích cho đồng nghiệp mới: tại sao payment system cần double-entry ledger thay vì chỉ update balance? Cho ví dụ khi single-entry fail.      |

### 💡 Feynman Prompt

> Hãy giải thích cho một Junior Developer: "Tại sao distributed rate limiter cần Redis Lua script thay vì GET + INCR riêng lẻ? Race condition xảy ra thế nào nếu 2 request đến cùng lúc?"

---

## 🔄 Spaced Repetition / Lặp Lại Ngắt Quãng

| Round | Thời điểm | Nội dung ôn                                                                                                            |
| ----- | --------- | ---------------------------------------------------------------------------------------------------------------------- |
| 1     | Ngày 1    | Đọc lại Overview + 6 Memory Hooks + Cold Call                                                                          |
| 2     | Ngày 3    | Trả lời Self-Check không mở tài liệu, review Layer 2 mechanics mỗi concept                                             |
| 3     | Ngày 7    | Làm 2 bài whiteboard prompt (chọn ngẫu nhiên), so sánh với answer                                                      |
| 4     | Ngày 14   | Mock interview: partner hỏi 1 problem random, trả lời đủ 7 bước (scope→estimate→HLD→API→deep dive→tradeoff→monitoring) |
| 5     | Ngày 30   | Review failure mode catalog + estimation workbook, tự estimate cho 1 bài mới                                           |
...
| File                                             | Liên kết                                                    |
| ------------------------------------------------ | ----------------------------------------------------------- |
| `04-be-system-design/01-design-framework.md`     | 5-step framework, estimation techniques, tradeoff reasoning |
| `04-be-system-design/02-classic-problems.md`     | URL shortener, chat, complementary problem set              |
| `04-be-system-design/04-distributed-patterns.md` | Distributed patterns applied across these problems          |
| `02-backend-knowledge/03-distributed-systems.md` | CAP, consensus, replication theory behind these designs     |
| `02-backend-knowledge/07-resilience-patterns.md` | Circuit breaker, retry, bulkhead used in every problem      |

### Cross-track

| File                                              | Liên kết                                                       |
| ------------------------------------------------- | -------------------------------------------------------------- |
| `shared/02-system-design/system-design-theory.md` | Fundamental design process, estimation formulas                |
| `shared/03-database/database-theory.md`           | Data modeling, indexing, consistency theory                    |
| `shared/04-security/01-security-fundamentals.md`  | PCI compliance, auth, threat modeling for Payment/Notification |
