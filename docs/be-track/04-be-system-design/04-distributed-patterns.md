...
**Tiki Order Checkout (thực tế):** Một đơn hàng cần: trừ inventory, charge payment, gửi invoice. Nếu payment success nhưng inventory đã hết giữa chừng → hệ thống monolith dùng DB transaction để rollback. Sau khi migrate sang microservices, không có distributed ACID — nếu payment charge thành công nhưng inventory service timeout, tiền bị charge mà hàng không trừ. Fix: Saga pattern với compensating transactions (hoàn tiền nếu inventory service fail).

**Bài học:** Distributed systems không có "distributed transactions" miễn phí. Mỗi pattern (Saga, 2PC, Outbox) là một tradeoff giữa consistency, availability, và complexity.

## What & Why / Cái Gì & Tại Sao

**Analogy:** Distributed patterns giống quy trình công ty nhiều phòng ban: không thể "undo" tất cả mọi thứ cùng lúc như xóa một file. Thay vào đó, cần "compensating actions": phòng tài chính hoàn tiền, phòng kho khôi phục hàng, phòng logistics hủy đơn — theo thứ tự ngược lại.

**Why it matters:** Mọi hệ thống microservices đều gặp vấn đề này. Saga và Outbox pattern là hai pattern phỏng vấn Senior bắt buộc phải biết.
