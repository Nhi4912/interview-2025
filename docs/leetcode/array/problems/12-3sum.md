...
# 3Sum / Tổng Ba Số Bằng 0

> **Track**: Shared | **Difficulty**: 🟡 Medium | **Pattern**: Sort + Two Pointers
> **Frequency**: 🔥 Tier 1 — Extension of Two Sum, extremely common
> **See also**: [Two Sum](./04-two-sum.md) | [Container With Most Water](./18-container-with-most-water.md)
...
i=0, nums[i]=-4, target = 0-(-4) = 4
  lo=1(-1), hi=5(2): sum=-1+2=1 < 4 → lo++
  lo=2(-1), hi=5(2): sum=-1+2=1 < 4 → lo++
  lo=3(0),  hi=5(2): sum=0+2=2  < 4 → lo++
  lo=4(1),  hi=5(2): sum=1+2=3  < 4 → lo++
  lo=5 >= hi=5 → done
  No triplet with -4

i=1, nums[i]=-1, target=1
  lo=2(-1), hi=5(2): sum=-1+2=1 == 1 → FOUND [-1,-1,2] ✓
    skip duplicates: lo++ until nums[lo]≠-1, hi-- until nums[hi]≠2
...
**LeetCode #15.** Given integer array, return all triplets `[nums[i], nums[j], nums[k]]` such that they sum to 0 and no duplicate triplets.
