...
**Analogy:** **Vietnamese:** Mỗi số trong mảng mã hóa `(depth, position, value)` theo format `DPV` (2 chữ số đầu là D và P, chữ số cuối là V). Dựng cây vào HashMap với key = `depth * 10 + pos`, sau đó DFS từ gốc — cộng giá trị vào path sum khi đến lá.
