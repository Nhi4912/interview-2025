...
| --- | ------------------------------------------------------------ | ---------- | ----------------------- | ---------------------------------------------------------------------------------- |
| 1   | Khi nào chọn SSG, ISR, SSR, CSR cho dự án thực tế?           | 🟡         | Rendering strategy      | Data freshness as criterion, e-commerce breakdown (homepage/product/search/cart)   |
| 2   | Sự khác biệt giữa getStaticProps và Server Component fetch?  | 🟡         | Pages vs App Router     | Per-fetch granularity, streaming advantage, execution model                        |
| 3   | Cách tối ưu LCP cho trang Next.js?                           | 🔴         | Performance             | Measure → Image priority + sizes → rendering strategy → font → scripts             |
| 4   | fallback: 'blocking' vs fallback: true trong getStaticPaths? | 🟡         | Static paths            | UX vs SEO trade-off, router.isFallback handling for 'true'                         |
| 5   | Thiết kế rendering strategy cho site kiểu Shopee?            | 🔴         | E-commerce architecture | Enumerates every page type with reason, middleware at edge, on-demand revalidation |
...
| #   | Loại           | Câu hỏi                                                                                                                                  |
| --- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | 🔍 Retrieval   | Viết ra 4 rendering strategies (SSG/ISR/SSR/CSR) với ví dụ 1 page type thực tế cho mỗi loại — không nhìn lại.                            |
| 2   | 🎨 Visual      | Vẽ lại decision tree "data freshness → strategy": từ "static forever" đến "real-time per request" — nhánh nào dẫn đến ISR, SSR, CSR?     |
| 3   | 🛠️ Application | Lazada product detail page cần SEO tốt và inventory fresh theo giờ. Chọn strategy nào? Thiết kế revalidation strategy cụ thể.            |
| 4   | 🐛 Debug       | `getStaticPaths` với `fallback: false` nhưng users gặp 404 cho product pages hợp lệ được thêm sau khi deploy. Nguyên nhân và 2 cách fix. |
| 5   | 🎓 Teach       | Giải thích ISR cho backend developer Go: "Nó hoạt động như cache với TTL, nhưng có gì khác so với Redis cache thông thường?"             |

### Key Points (tự kiểm tra)

| #   | Key Point                                                                                                                                                                            |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | SSG: landing page/blog (vĩnh viễn static). ISR: product catalog (revalidate theo giờ). SSR: user dashboard (dynamic per request). CSR: admin panel (interactive, post-login).        |
| 2   | "Static forever" → SSG. "Mostly static, update periodically" → ISR. "Dynamic per user/request" → SSR. "Interactive, no SEO needed" → CSR. Decision: freshness × SEO × interactivity. |
| 3   | ISR với `revalidate: 3600` (inventory fresh mỗi giờ) + `revalidateTag('product-inventory')` khi inventory thay đổi trong CMS. SEO từ pre-generated HTML.                             |
| 4   | `fallback: false` = chỉ pre-generate paths đã biết lúc build; new paths → 404. Fix (1): `fallback: 'blocking'` (SSR cho new paths). Fix (2): ISR App Router (auto-handle).           |
| 5   | Giống Redis cache với TTL — serve stale khi hết TTL. Khác: Next.js regenerate page HTML trong background (stale-while-revalidate), Redis trả empty và bạn phải rebuild.              |
...
- [App Router & Server Components](./01-app-router-server-components.md) — RSC architecture detailed within this rendering model
- [Data Fetching](./02-data-fetching.md) — fetch strategies mapped to each rendering mode
- [App Router Fundamentals](./04-nextjs-fundamentals-appRouter.md) — routing conventions that drive rendering decisions
- [Next.js Fundamentals](./00-nextjs-fundamentals.md) — entry-level concepts before architecture patterns

### Khác track (Cross-track)
- [Core Web Vitals](../06-browser-performance/01-core-web-vitals.md) — rendering strategy choice directly impacts LCP/INP
- [React Fundamentals](../03-react/01-react-fundamentals.md) — React model Next.js is built on top of
- [Bundle Optimization](../06-browser-performance/03-bundle-optimization.md) — code splitting complements Next.js rendering strategies
