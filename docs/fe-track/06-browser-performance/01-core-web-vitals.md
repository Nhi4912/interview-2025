...
Old metrics (DOMContentLoaded, onload) fired before meaningful content appeared. LCP tracks when the user actually _sees_ the main content.
...
| --- | ------------------------------------------------------------ | ---------- | ------------------------------------ | ----------------------------------------------------------------------------------- |
| 1   | Core Web Vitals là gì? Tại sao quan trọng?                   | 🟢 Junior  | LCP / INP / CLS metrics & thresholds | Biết INP thay FID từ 3/2024; giải thích tại sao mỗi metric được Google chọn         |
| 2   | Page LCP = 4s. Walk me through debugging.                    | 🟡 Mid     | LCP debugging methodology            | Systematic: TTFB → resource loading → rendering; Lighthouse shows exact LCP element |
| 3   | Tại sao INP thay thế FID?                                    | 🟡 Mid     | FID vs INP distinction               | "p98" (98th percentile); FID chỉ đo input delay, không đo processing + presentation |
| 4   | CLS = 0.3 (poor). Identify và fix.                           | 🟡 Mid     | CLS root causes & fixes              | `hadRecentInput` filter; dùng `.sources` API để tìm shifting elements               |
| 5   | Measure Core Web Vitals in production — Real User Monitoring | 🔴 Senior  | RUM setup & production measurement   | `keepalive` for page-unload safety; CLS integer encoding; sampling strategy         |
...
| #   | Loại           | Câu hỏi                                                                                                              |
| --- | -------------- | -------------------------------------------------------------------------------------------------------------------- |
| 1   | 🔍 Retrieval   | Liệt kê 3 Core Web Vitals, target threshold "Good", và main cause của điểm kém cho mỗi metric — từ trí nhớ.          |
| 2   | 🎨 Visual      | Vẽ INP breakdown: 3 phases (Input Delay, Processing Time, Presentation Delay) — chỉ rõ cách fix từng phase.          |
| 3   | 🛠️ Application | Image gallery với 20 ảnh không có `width`/`height` attributes. Metrics nào bị ảnh hưởng và tại sao? Fix như thế nào? |
| 4   | 🐛 Debug       | Lighthouse: LCP = 4.2s, LCP element = `<img class="hero">`. Bạn trace 5 bước debug gì theo thứ tự?                   |
| 5   | 🎓 Teach       | Giải thích CLS cho PM — tại sao layout shift là bad UX và ảnh hưởng gì đến conversion rate?                          |

### Key Points (tự kiểm tra)

| #   | Key Point                                                                                                                                                                                                                          |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | LCP ≤2.5s (Largest Contentful Paint — slow server/image); INP ≤200ms (Interaction to Next Paint — long JS tasks); CLS ≤0.1 (Cumulative Layout Shift — images no dimensions, dynamic content injection).                            |
| 2   | Input Delay (main thread busy → dùng scheduler/break tasks) → Processing Time (event handler nặng → tách, defer, web worker) → Presentation Delay (sync layout → batch DOM reads/writes, avoid layout thrashing).                  |
| 3   | **CLS bị ảnh hưởng** — ảnh không có kích thước → browser không reserved space → ảnh tải xong đẩy content xuống. Fix: thêm `width`/`height` attributes hoặc `aspect-ratio` CSS + `loading="lazy"` cho ảnh dưới fold.                |
| 4   | (1) TTFB: server response chậm? (2) Resource discovery: img có trong HTML ngay hay load qua JS? (3) Load time: kích thước/format? (4) Render-blocking: CSS/font delay? (5) Kiểm tra `loading="lazy"` bị set trên hero image không? |
| 5   | CLS = UI bị nhảy khi tải — user định click nút "Mua" nhưng quảng cáo load → nút bị đẩy xuống → click nhầm vào ad → frustrating. Conversion giảm vì users mất trust, bounce rate tăng.                                              |
...
- [React Performance](./02-react-performance.md) — React-specific optimizations that improve INP and LCP
- [Bundle Optimization](./03-bundle-optimization.md) — reducing JS payload directly lowers LCP and INP
- [Web Performance Comprehensive](./04-web-performance-comprehensive.md) — full performance strategy covering all CWV metrics
- [Rendering Optimization Theory](./05-rendering-optimization-theory.md) — Critical Rendering Path behind LCP and CLS

### Khác track (Cross-track)
- [Next.js Architecture](../04-nextjs/03-nextjs-architecture.md) — rendering strategy selection (SSG/ISR/SSR) impacts CWV scores
- [Data Fetching](../04-nextjs/02-data-fetching.md) — caching and streaming strategies directly improve LCP
- [React Performance Optimization](../03-react/09-performance-optimization.md) — memoization and lazy loading improve INP scores
