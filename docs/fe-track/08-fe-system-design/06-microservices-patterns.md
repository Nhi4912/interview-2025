...
async function callUserService(userId) {
  ...
  const response = await fetch(`http://${service.address}:${service.port}/users/${userId}`);
  return response.json();
}
```
...
- → Mở đầu trả lời: _"API Gateway là single entry point để FE không cần biết topology của backend. Nó xử lý auth, rate limiting, và request aggregation tập trung — nhưng không chứa business logic."_
...
- → Mở đầu trả lời: _"BFF pattern giải quyết vấn đề mỗi client có data requirement khác nhau. Thay vì một API cố gắng phục vụ tất cả, mỗi client type có một BFF riêng — một lớp mỏng aggregate và transform data từ microservices theo đúng nhu cầu của client đó."_
...
- → Mở đầu trả lời: _"Circuit Breaker pattern ngăn cascading failure bằng cách fail fast khi downstream service không healthy. Quan trọng là phải có fallback strategy — cached data hoặc graceful degradation — để UX vẫn hoạt động khi circuit mở."_
...
- → Mở đầu trả lời: _"Service Mesh giải quyết vấn đề duplicate infrastructure code ở mọi service bằng cách dùng sidecar proxy. Với FE perspective, điều quan trọng là hiểu mesh xử lý inter-service concerns — FE vẫn phải own UI-level resilience."_
...
- → Mở đầu trả lời: _"Micro-frontend áp dụng nguyên tắc microservices cho UI — mỗi team owns một domain slice và deploy độc lập. Key challenge là shared dependencies (React singleton) và consistent UX qua design system chung."_
...
async function handleInventoryReserved(event) {
  await Order.updateOne({ _id: event.orderId }, { status: "CONFIRMED" });
}

// Order Service - Compensate
async function handleInventoryFailed(event) {
  await Order.updateOne({ _id: event.orderId }, { status: "CANCELLED" });
}
```
...
class EventStore {
  ...
  async getNextVersion(streamId) {
    const lastEvent = await Event.findOne({ streamId }).sort({ version: -1 });
...
- → Mở đầu trả lời: _"CQRS ở FE level xuất hiện rõ nhất trong optimistic updates — FE update read model ngay lập tức khi user action, rồi reconcile với server response. Điều quan trọng là FE phải handle eventual consistency: data có thể stale trong khoảng thời gian ngắn."_
...
class OrderProjection {
  ...
  async handleOrderCancelled(event) {
    await OrderReadModel.updateOne({ orderId: event.orderId }, { status: "CANCELLED" });
  }
}
```
...
| --- | ----------------------------------------------------------- | ---------- | ------------------------------- | ---------------------------------------------------------------------------------------------------- |
| 1   | Khi nào dùng microservices vs monolith?                     | 🟡 Mid     | Monolith-first strategy         | "Monolith first" is best practice; migrate when team size, deploy conflicts, or scaling needs appear |
| 2   | Xử lý distributed transactions trong microservices thế nào? | 🔴 Senior  | Saga pattern for distributed TX | 2PC not suitable (blocking + SPOF); Saga with compensating transactions; idempotency required        |
| 3   | Giải thích Circuit Breaker pattern                          | 🟡 Mid     | Circuit Breaker resilience      | 3 states with transitions; mandatory fallback strategy (cached data, default response)               |
| 4   | Phân biệt choreography và orchestration trong Saga          | 🔴 Senior  | Saga coordination styles        | When to use each + trade-off: choreography (harder to debug) vs orchestration (SPOF)                 |
| 5   | Triển khai service discovery thế nào?                       | 🟡 Mid     | Service discovery mechanisms    | Client-side vs server-side; Kubernetes built-in discovery; health checks; stale registry risk        |
| 6   | Giải thích BFF pattern — khi nào dùng thay vì API Gateway?  | 🔴 Senior  | Backend for Frontend pattern    | BFF solves one-size-fits-all API; FE team owns BFF; GraphQL as alternative                           |
...
| #   | Loại           | Câu hỏi                                                                                                                        |
| --- | -------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 1   | 🔍 Retrieval   | Giải thích BFF pattern từ trí nhớ — vấn đề gì nó giải quyết và khác API Gateway như thế nào?                                   |
| 2   | 🎨 Visual      | Vẽ sơ đồ API Gateway vs BFF từ trí nhớ — chỉ rõ ai own mỗi layer và data flow của mỗi client type.                             |
| 3   | 🛠️ Application | Mobile app cần data nhẹ hơn, web cần data phong phú hơn từ cùng backend. BFF hay GraphQL? Khi nào chọn cái nào?                |
| 4   | 🐛 Debug       | FE gọi 5 microservices cho 1 trang, mỗi cái 200ms — page load 1s+. Sequential hay parallel? Bottleneck ở đâu? Fix theo 3 cách. |
| 5   | 🎓 Teach       | Giải thích Circuit Breaker cho người không biết lập trình bằng một analogy đời thường.                                         |

### Key Points (tự kiểm tra)

| #   | Key Point                                                                                                                                                                                                                                                              |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | BFF (Backend for Frontend): dedicated aggregation layer per client type, owned bởi FE team, custom-tailored cho exact data needs. API Gateway: single entry point routing requests to services, owned bởi infra/backend team, generic routing không có business logic. |
| 2   | API Gateway: Client → Gateway → Service A/B/C (1 gateway, nhiều clients). BFF: Mobile App → Mobile BFF → Services; Web App → Web BFF → Services (mỗi client có BFF riêng, custom aggregate data shape).                                                                |
| 3   | **BFF** khi mobile/web có data shape khác nhau rõ ràng, FE team control được backend layer, performance cần optimized per client. **GraphQL** khi frontend cần query flexibility, schema evolve thường xuyên, hoặc 1 backend serve nhiều client types chưa biết trước. |
| 4   | Sequential = 5×200ms = 1s. Fix: (1) `Promise.all()` — parallel calls = ~200ms; (2) BFF aggregation layer — server-to-server calls nhanh hơn client-to-server; (3) Cache common data (CDN hoặc Redis) — reduce origin calls.                                            |
| 5   | Circuit Breaker = cầu dao điện — khi điện bị chập (service fail nhiều lần), cầu dao tự ngắt (open state), không cho thêm điện qua tránh cháy nhà. Sau vài phút tự test lại (half-open), nếu OK thì đóng lại (closed).                                                  |
...
- [Architecture Patterns](./01-architecture-patterns.md) — micro-frontends architecture liên quan chặt với microservices backend
- [Scalability](./02-scalability.md) — microservices là một chiến lược scale team và deploy
- [Microservices Overview](./04-microservices.md) — fundamentals và trade-offs của microservices approach
- [Caching](./03-caching.md) — caching strategies trong microservices environment

### Khác track (Cross-track)
- [Backend Microservices](../../be-track/02-backend-knowledge/02-microservices.md) — service decomposition, inter-service communication
- [API Design](../../be-track/02-backend-knowledge/01-api-design.md) — REST/GraphQL API design cho microservices
- [System Design Theory](../../shared/02-system-design/system-design-theory.md) — distributed systems principles behind microservices
