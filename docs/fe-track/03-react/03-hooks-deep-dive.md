...
| --- | ---------------------------------------------------- | ---------- | -------------------- | --------------------------------------------------- |
| 1   | Automatic batching React 18 vs React 17?             | 🟢 Junior  | React 18             | Async events cũng được batch trong React 18         |
| 2   | `setState(prev => ...)` — khi nào cần dùng?          | 🟢 Junior  | State updates        | Tránh stale state trong closures                    |
| 3   | `useEffect` cleanup — tại sao cần, thiếu thì sao?    | 🟡 Mid     | Side effects         | Memory leaks + subscription cleanup                 |
| 4   | 3 dạng dependency array — khi nào dùng mỗi dạng?     | 🟢 Junior  | useEffect            | `[]`, `undefined`, `[deps]` use cases               |
| 5   | Stale closure trong hooks — 3 cách fix               | 🟡 Mid     | Closure              | Functional updater + ref + ESLint rule              |
| 6   | `useRef` — 2 use cases, khác `useState` như thế nào? | 🟡 Mid     | Refs                 | DOM access vs mutable value không trigger re-render |
| 7   | `useMemo` vs `useCallback` — khi nào dùng?           | 🟡 Mid     | Memoization          | Biết khi nào memo có lợi vs overhead                |
| 8   | `useReducer` vs `useState` — khi nào chọn cái nào?   | 🟡 Mid     | State management     | Complex transitions + predictability                |
| 9   | `useContext` gây re-render như thế nào? Fix?         | 🔴 Senior  | Context optimization | Split context + memoize value                       |
| 10  | Rules of Hooks — tại sao React yêu cầu?              | 🟡 Mid     | Hook internals       | Linked list call order dependency                   |
| 11  | `useLayoutEffect` vs `useEffect`                     | 🔴 Senior  | Render timing        | Synchronous vs async, DOM reads trước paint         |
| 12  | Design `useFetch` với cancel + loading state         | 🔴 Senior  | Hook design          | AbortController + cleanup pattern                   |
...
| #   | Loại           | Câu hỏi                                                                                                                                      |
| --- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | 🔍 Retrieval   | Vẽ linked list model của hooks từ trí nhớ. Tại sao conditionals phá vỡ hooks? React dựa vào gì để match hook với state?                      |
| 2   | 🎨 Visual      | Trace `setCount(count + 1)` gọi 2 lần trong batch vs `setCount(c => c + 1)` gọi 2 lần — kết quả khác nhau thế nào và tại sao?                |
| 3   | 🛠️ Application | Component fetch user data theo `userId`. Viết `useEffect` đúng cách: loading state, error handling, và cancel request khi `userId` thay đổi. |
| 4   | 🐛 Debug       | `useEffect` với `[]` deps nhưng chạy mỗi render vì dependency là object được tạo inline trong render. Identify và fix.                       |
| 5   | 🎓 Teach       | Giải thích stale closure trong `setInterval` + `useEffect` cho teammate — dùng "bức ảnh chụp lại" analogy.                                   |

### Key Points (tự kiểm tra)

| #   | Key Point                                                                                                                                                                |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Hooks lưu trong linked list gắn với component instance. Conditional thay đổi thứ tự → hook N match sai state. React dựa vào call order (không phải tên biến).            |
| 2   | Batch: cả 2 call đọc cùng `count=0` → state cuối là `1`. Functional: `c => c+1` nhận latest state → kết quả là `2`. Dùng functional update khi new state depends on old. |
| 3   | `const controller = new AbortController()`. Fetch với `signal: controller.signal`. Cleanup: `return () => controller.abort()`. Deps: `[userId]`.                         |
| 4   | Object `{ id: 1 }` tạo mới mỗi render → reference khác nhau → deps thay đổi → re-run effect. Fix: move object ra ngoài component hoặc dùng `useMemo`.                    |
| 5   | `useEffect` chụp lại giá trị tại thời điểm chạy (như ảnh). `setInterval` giữ tham chiếu đến bức ảnh cũ. Fix: functional update không cần đọc stale value từ closure.     |
...
- [React Fundamentals](./01-react-fundamentals.md) — rendering model that hooks are built on
- [Hooks Comprehensive](./07-hooks-comprehensive.md) — broader hook taxonomy and concurrency hooks
- [State Management](./05-state-management.md) — when hooks alone aren't enough
- [Performance Optimization](./09-performance-optimization.md) — useMemo/useCallback tuning strategies

### Khác track (Cross-track)
- [Closures](../01-javascript/03-closures.md) — stale closure bugs explained from first principles
- [React TypeScript](../02-typescript/05-react-typescript.md) — typing custom hooks and generics
- [Data Structures Theory](../../shared/01-cs-fundamentals/data-structures-theory.md) — hooks linked-list model under the hood
