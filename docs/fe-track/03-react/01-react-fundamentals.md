...
return <label htmlFor="email" className="label">Email</label>;
...
- [ ] **Retrieval**: Vẽ React's rendering pipeline từ trí nhớ: state change → Render Phase → Commit Phase → browser paint.
- [ ] **Visual**: Giải thích tại sao `{0 && <Component />}` render "0" — vẽ luồng từ JSX → createElement → render.
- [ ] **Application**: List 10,000 users với search filter. State là array. Tại sao key=index bị sai? Khi nào index là OK?
- [ ] **Debug**: Component có stale closure bug trong `useEffect` với `setInterval`. Identify bug + fix không thay đổi interval.
- [ ] **Teach**: Giải thích Virtual DOM cho người chỉ biết HTML/CSS — dùng ví dụ Word document và track changes.
