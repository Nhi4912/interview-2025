...
- [ ] **Retrieval**: Giải thích concurrent rendering theo ví dụ "surgeon" — tại sao React 17 "blocks" và React 18 không?
- [ ] **Visual**: Vẽ lại component tree với Server Component ở root, Client Component ở leaf — điều gì xảy ra ở ranh giới khi pass function prop từ server sang client?
- [ ] **Application**: Team dùng React Compiler — bạn có xóa tất cả `useMemo`/`useCallback` trong codebase không? Còn cái gì cần giữ?
- [ ] **Debug**: Client Component import một Server Component trực tiếp — lỗi gì xảy ra? Fix thế nào?
- [ ] **Teach**: Giải thích `useOptimistic` cho backend developer: "Nó khác với `setState(newValue)` ở chỗ nào — và điều gì xảy ra khi server trả về lỗi?"
