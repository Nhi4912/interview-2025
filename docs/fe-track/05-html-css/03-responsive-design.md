...
| --- | ----------------------------------- | ---------- | ----------------- | ------------------------------------------------------------------ |
| 1   | Tại sao CSS mobile-first?           | 🟢         | Mobile-first      | Additive not subtractive, min-width vs max-width                   |
| 2   | srcset vs picture?                  | 🟢         | Responsive images | srcset = hint; picture = directive (browser must follow)           |
| 3   | Container queries vs media queries? | 🟡         | Container queries | Same component in sidebar vs main — container-type:inline-size     |
| 4   | clamp() cho fluid typography?       | 🟡         | Fluid typography  | Three-arg structure, eliminates font-size media queries            |
| 5   | dvh, svh, lvh?                      | 🟡         | Viewport units    | vh jumps on mobile, dvh = dynamic, svh = safe minimum              |
| 6   | Responsive anti-patterns?           | 🔴         | Anti-patterns     | display:none still downloads, user-scalable=no = WCAG violation    |
| 7   | Responsive design audit?            | 🔴         | Audit methodology | 5-layer systematic: viewport → layout → images → typography → a11y |
...
| #   | Loại           | Câu hỏi                                                                                                                                            |
| --- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | 🔍 Retrieval   | Viết từ trí nhớ CSS cho mobile-first card grid với 3 breakpoints — `grid-template-columns` cho mỗi breakpoint — không nhìn lại.                    |
| 2   | 🎨 Visual      | Vẽ diagram cho `clamp(1rem, 0.9rem + 0.5vw, 1.5rem)` — giá trị tại 320px viewport và tại 1440px viewport là bao nhiêu?                             |
| 3   | 🛠️ Application | Component card hiển thị đúng ở viewport 800px nhưng sai ở sidebar 300px dù cùng viewport. Cách fix?                                                |
| 4   | 🐛 Debug       | `position: fixed` modal không full height trên mobile iPhone, bị cắt bởi address bar. Nguyên nhân? Fix?                                            |
| 5   | 🎓 Teach       | Giải thích `container queries` cho một backend developer chưa biết CSS hiện đại — tại sao media queries không đủ cho component có thể tái sử dụng? |

### Key Points (tự kiểm tra)

| #   | Key Point                                                                                                                                                                                                                       |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Mobile-first: `grid-template-columns: 1fr` (default) → `@media (min-width: 768px) { grid-template-columns: repeat(2, 1fr) }` → `@media (min-width: 1024px) { grid-template-columns: repeat(3, 1fr) }`.                          |
| 2   | `clamp(min, preferred, max)`: tại 320px → 0.9rem + 0.5\*(320/100) = 0.9 + 1.6 = 2.5rem → clamp xuống `1.5rem`; tại 1440px → 0.9 + 7.2 = 8.1rem → clamp xuống `1.5rem`. Preferred thực ra tính bằng vw nên cần viewport nhỏ hơn. |
| 3   | Media query dựa trên viewport width, không phải kích thước container — sidebar 300px trong viewport 800px không trigger breakpoint. Fix: dùng Container Queries `@container (min-width: 400px) { ... }`.                        |
| 4   | Safari iOS: `100vh` không tính address bar → modal bị cắt. Fix: dùng `height: 100dvh` (dynamic viewport height) hoặc `min-height: -webkit-fill-available` + `safe-area-inset`.                                                  |
| 5   | Media query = "nếu màn hình rộng 800px thì..."; Container query = "nếu hộp chứa component rộng 400px thì...". Component có thể nằm trong sidebar (hẹp) hoặc main content (rộng) trên cùng viewport → container query mới đúng.  |
