...
@container card-grid (min-width: 600px) {
  .card { /* applies only when inside card-grid container */ }
}
...
**Common Mistakes:**
- Quên khai báo `container-type` trên parent → `@container` không hoạt động
- Dùng `container-type: size` khi chỉ cần `inline-size` — `size` cần explicit height trên container
- Đặt `container-type` trực tiếp trên element cần query (phải đặt trên **parent**)
...
**Common Mistakes:**
- Nhầm rằng layer sau có **lower** priority — ngược lại, layer sau = higher priority
- Đặt utility classes trong layer → bị override bởi unlayered styles (dùng unlayered cho utils hoặc tạo layer riêng cuối cùng)
- Khai báo layer order sau khi đã dùng `@layer {}` → order được set by first `@layer` declaration
...
.btn { min-height: max(44px, 2.75rem); }
```
...
**Common Mistakes:**
- `clamp()` argument order sai: `clamp(min, preferred, max)` — min phải <= max hoặc kết quả là undefined
- Dùng `clamp()` mà không test trên small/large viewport → preferred value có thể violate bounds
- Nhầm `min()` và `max()`: `min(a, b)` trả giá trị **nhỏ hơn** (dùng để cap maximum), `max(a, b)` trả giá trị **lớn hơn** (dùng để ensure minimum)
...
```css
h1 { font-size: clamp(1.5rem, 1rem + 2vw, 3rem); }  /* fluid typography */
.container { width: min(90vw, 1200px); margin-inline: auto; }
.btn { min-height: max(44px, 2.75rem); }              /* WCAG touch target */
```
...
.accent { color: hsl(250, 80%, 55%); }   /* fallback */
.accent { color: oklch(55% 0.25 270); }  /* overrides if supported */
```
...
.card {
  ...
  & .title { font-size: 1.25rem; }   /* .card .title */
  &:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
  &--featured { border-color: gold; } /* .card--featured */

  @media (max-width: 768px) { padding: 0.5rem; }

  .dark-theme & { background: #333; color: #fff; } /* .dark-theme .card */
}
```
...
.card:not(:has(img)) { display: flex; flex-direction: column; }
...
:root {
  ...
  --blue-light: oklch(80% 0.10 240);  /* same hue, just lighter + less saturated */
...
.accent { color: hsl(250, 100%, 50%); }  /* fallback */
.accent { color: oklch(50% 0.35 270); }  /* wide gamut if supported */
```
...
.progress-bar {
  ...
  width: 100%; height: 3px; background: blue;
...
.accent { color: hsl(250, 80%, 55%); }   /* sRGB */
.accent { color: oklch(55% 0.25 270); }  /* wide gamut — overrides if supported */

/* Subgrid with fallback */
.grid-item { grid-template-rows: auto 1fr auto; }  /* own tracks */
@supports (grid-template-rows: subgrid) {
  .grid-item { grid-template-rows: subgrid; }       /* inherit if supported */
}
```
...
1. Không nhìn notes: giải thích sự khác nhau giữa media query và container query trong 2 câu.
2. `@layer reset, base, components` — layer nào có priority cao nhất? Style nào thắng tất cả?
3. `:has()` có thể làm gì mà CSS trước đây không thể? Cho 2 ví dụ use case.
4. Tại sao `oklch` tốt hơn `hsl` cho design system palettes?
5. Scroll-driven animations chạy trên thread nào? Tại sao quan trọng?
