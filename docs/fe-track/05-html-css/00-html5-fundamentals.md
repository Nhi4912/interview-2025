...
- [ ] **Retrieval**: Viết từ trí nhớ sự khác biệt giữa `async` và `defer` — thứ tự execution, khi nào nên dùng cái nào — không nhìn lại.
- [ ] **Visual**: Vẽ timeline HTML parsing cho 3 trường hợp: `<script>` thường, `async`, và `defer` — chỉ rõ khi nào parsing bị block.
- [ ] **Application**: Form với `<input type="email" required>` — khi user submit email không hợp lệ, validation chạy hay không? Làm sao để custom error message?
- [ ] **Debug**: `<header role="banner">` — `role="banner"` có cần thiết không? Khi nào thì `<header>` KHÔNG tự động là `banner`?
- [ ] **Teach**: Giải thích sự khác biệt giữa `<section>` và `<div>` cho một junior developer bằng một câu tương tự cụ thể.
