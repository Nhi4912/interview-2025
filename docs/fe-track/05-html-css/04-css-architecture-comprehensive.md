...
| --------------------------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Adding more classes to beat specificity | Creates escalation war — next override needs even more classes, no end                   | Lower specificity of conflicting rule (use `:where()` to zero-out score) |
| `!important` to fix specificity war     | Treats symptom not root cause — `!important` itself becomes impossible to override       | Reduce original selector specificity at the source                       |
| Thinking 11 classes beats 1 ID          | Each specificity column is independent — `(1,0,0)` always beats `(0,99,99)`              | Columns don't carry over — 1 ID always wins over any class count         |
| Specificity = "importance"              | They're separate cascade axes — `!important` changes origin track, not specificity score | `!important` is cascade origin; specificity is a separate algorithm      |
...
.container {
  overflow: hidden;
} /* triggers BFC → clears floats */
.flex-parent {
  display: flex;
} /* flex context = BFC for children */
...
| Sai lầm                                          | Tại sao sai                                                                                | Đúng là                                                                            |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| `width: calc(100% - 40px)` to compensate padding | Fragile — any padding change breaks the calc; not semantic intent                          | Set `box-sizing: border-box` globally — `width` includes padding and border        |
| Confused by adjacent margin collapse             | Adjacent vertical margins merge into the larger value, not add — causes unexpected spacing | Know top/bottom margins of adjacent blocks merge; use BFC or padding to prevent    |
| `overflow: hidden` on parent for floats          | Creates unintended clipping side effect on child elements that overflow visually           | Use `display: flow-root` — explicit BFC, no overflow clipping side effect          |
| `height: 100%` on child without parent height    | Percentage height requires an ancestor with explicit height — resolves to `auto` otherwise | Parent needs explicit `height` or `min-height` for percentage child height to work |
...
| Sai lầm                                        | Tại sao sai                                                                                                | Đúng là                                                                            |
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Component overriding generic with `!important` | Signals wrong layer placement — component specificity should naturally exceed generic without `!important` | Fix component's specificity — it should naturally be higher than generic           |
| Utilities in Objects layer                     | Utilities need highest priority; Objects layer can't beat lower-specificity rules in layers above          | Keep utilities in the last layer — they must be able to override anything          |
| ITCSS with BEM conflict                        | They serve different concerns — ITCSS organizes files by specificity; BEM organizes class names            | ITCSS = file organization; BEM = naming convention — they complement, not conflict |
| All CSS in one file                            | Without file separation, layer order is impossible to maintain as codebase grows                           | ITCSS requires file separation per layer to enforce cascade order                  |
...
Box model định nghĩa kích thước thực tế của element. `content-box`: `width: 200px; padding: 20px` → rendered 240px (surprise!). `border-box`: `width: 200px` → luôn là 200px dù padding hay border bao nhiêu. Lý do ưu tiên `border-box`: khi design nói "button rộng 200px", họ muốn _thấy_ 200px, không phải 200 + 40px padding.

**💡 Dấu hiệu trả lời tốt / Interview Signal:**

- ✅ Strong: Demonstrates the math difference with a concrete example, explains _why_ `border-box` eliminates `calc(100% - 40px)` hacks, mentions the universal selector reset.
...
- ❌ Weak: Says "just use BEM" without explaining _why_ BEM prevents wars (flat specificity), doesn't mention `@layer`.
...
- ✅ Strong: Uses a framework (comparison matrix) rather than personal preference, identifies the _actual_ bottleneck at 200-dev scale (design consistency, not CSS power), names the Tailwind SSR caveat and the CSS-in-JS runtime cost, mentions the white-labeling exception.
...
2. "Then I check _why_ the wrong rule wins: is it specificity (A,B,C score), origin (`!important`?), or source order (same specificity, later rule wins)?"
...
| #   | Loại           | Câu hỏi                                                                                                                                    |
| --- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | 🔍 Retrieval   | Viết specificity score cho selector `#nav .item:hover` từ trí nhớ. Kết quả là bao nhiêu?                                                   |
| 2   | 🎨 Visual      | Vẽ ITCSS inverted triangle từ trí nhớ với đủ 7 layers theo thứ tự từ đỉnh xuống — layer nào dùng `!important`?                             |
| 3   | 🛠️ Application | Selector `.modal .submit-btn.active` (0,3,0) đang override `.btn-primary` (0,1,0). Dùng 2 cách khác nhau để fix mà không thêm specificity. |
| 4   | 🐛 Debug       | `.card__title` không nhận `color: red` — DevTools Computed cho thấy rule bị crossed out với (0,1,0) vs (0,2,0). Nguyên nhân? Fix?          |
| 5   | 🎓 Teach       | Giải thích ITCSS cho teammate không biết CSS architecture bằng 1-2 câu liên tưởng đơn giản, không dùng technical terms.                    |

### Key Points (tự kiểm tra)

| #   | Key Point                                                                                                                                                                                                               |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `#nav` = (1,0,0), `.item` = (0,1,0), `:hover` = (0,1,0) → tổng = **(1,2,0)**.                                                                                                                                           |
| 2   | Settings → Tools → Generic → Elements → Objects → Components → **Utilities** (từ generic/low-specificity đến specific/high-specificity); chỉ layer Utilities được dùng `!important` vì là intentional highest priority. |
| 3   | (1) Dùng `:where(.modal .submit-btn.active)` → zero-out specificity về (0,0,0); (2) Dùng `@layer` để control cascade layer order thay vì specificity battle.                                                            |
| 4   | Có rule khác với specificity cao hơn (0,2,0) đang override — ví dụ `.card .card__title`. Fix: tách selector về flat BEM level (0,1,0), hoặc dùng `@layer` để override ở layer cao hơn.                                  |
| 5   | ITCSS như tòa nhà — tầng dưới cùng là móng chung (Reset/Generic), tầng trên cùng là trang trí đặc thù (Utilities). Rule ở tầng trên luôn thắng — không bao giờ lộn xộn "cái gì đang override cái gì".                   |

> 🎯 **Feynman Prompt:** Giải thích CSS specificity cho người không biết lập trình — dùng liên tưởng "tòa án phân xử tranh chấp". Tại sao 1 thẩm phán (ID) luôn thắng 100 nhân viên tòa (class)?
