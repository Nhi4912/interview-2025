...
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| BEM elements nested inside elements: `.card__header__title`      | Element names không reflect DOM nesting trong BEM — tên dài vô nghĩa, khó refactor khi DOM thay đổi | Dùng `.card__title` — element thuộc block trực tiếp, không thuộc element khác        |
| BEM modifier không có base class: `<div class="card--featured">` | Modifier cần base class để có đầy đủ styles — thiếu base thì modifier mất context và style          | Luôn include base class: `<div class="card card--featured">`                         |
| Utility-first cho mọi thứ kể cả complex interaction states       | Logic tương tác phức tạp trong HTML utilities → unmaintainable, không thể search hoặc debug         | Dùng BEM-named semantic classes cho complex states; utilities chỉ cho layout/spacing |
...
| Sai lầm                                                                     | Tại sao sai                                                                                       | Đúng là                                                                              |
| --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Components trong sai layer (utility trong component layer)                  | Override intent bị mất — utility phải thắng component, không thể nếu nằm cùng hoặc layer thấp hơn | Đặt đúng layer: utilities luôn ở layer sau cùng với priority cao nhất                |
| Deeply nested selectors trong ITCSS objects (`.o-wrapper > .c-card .c-btn`) | Tăng specificity không cần thiết, phá vỡ mục đích ITCSS là giữ specificity thấp và đồng đều       | Objects chỉ dùng single class selectors — không nest component selectors bên trong   |
| Dùng ITCSS trong project nhỏ                                                | Overhead của 7 layers không xứng đáng cho single-dev hoặc small team — là over-engineering        | Chỉ áp dụng ITCSS khi team > 3 người hoặc codebase đủ lớn để justify file separation |
...
| Sai lầm                                                                             | Tại sao sai                                                                                    | Đúng là                                                                                  |
| ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Bỏ qua semantic layer — component reference primitive trực tiếp (`var(--blue-500)`) | Theming yêu cầu thay đổi từng component — không scale khi thêm dark mode hoặc brand mới        | Tạo semantic tokens `--color-primary` → component dùng semantic, chỉ update 1 file theme |
| Tokens quá granular (`--btn-border-top-left-radius`)                                | Token sprawl — khó maintain, không ai nhớ token nào còn được dùng hay đã deprecated            | Chỉ tạo tokens cho values thay đổi theo theme; không token-ize từng CSS property         |
| Không scope component tokens                                                        | Component tokens không thể bị override contextually — theming per-component instance không thể | Để component tokens có thể bị override bởi parent context hoặc theme scope               |
...
| #   | Loại           | Câu hỏi                                                                                                                                 |
| --- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | 🔍 Retrieval   | Tại sao BEM selectors ở level (0,1,0) quan trọng cho maintainability? Và 3 tầng design tokens là gì — tại sao cần tầng semantic ở giữa? |
| 2   | 🎨 Visual      | Vẽ ITCSS inverted triangle với đủ 7 layers theo thứ tự — ghi chú layer nào được phép dùng `!important` và tại sao.                      |
| 3   | 🛠️ Application | Nêu 3 bước đầu tiên khi migrate legacy CSS codebase 200 global files — theo thứ tự ưu tiên và lý do.                                    |
| 4   | 🐛 Debug       | Dark mode cần update 50 component files mỗi lần thêm theme mới. Đây là dấu hiệu của architecture problem gì? Cách fix?                  |
| 5   | 🎓 Teach       | Giải thích design tokens cho một backend developer chỉ biết programming variables — sự khác biệt giữa semantic và primitive token.      |

### Key Points (tự kiểm tra)

| #   | Key Point                                                                                                                                                                                                                                                                                                                |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | BEM (0,1,0): không có ID → không tạo high-specificity wars; mọi override chỉ cần thêm 1 class; dễ refactor. 3 tầng: Primitive (`--color-blue-500: #3b82f6`) → Semantic (`--color-primary: var(--color-blue-500)`) → Component (`--btn-bg: var(--color-primary)`). Tầng semantic = layer của ý nghĩa, không phải giá trị. |
| 2   | Settings → Tools → Generic → Elements → Objects → Components → **Utilities**. Chỉ layer Utilities (`!important`) vì là intentional highest-priority overrides — utility classes như `.hidden` cần thắng mọi thứ.                                                                                                         |
| 3   | (1) Audit: specificity graph + CSS coverage → tìm dead code và hotspots; (2) Wrap legacy CSS vào `@layer legacy` ngay → new code tự động thắng; (3) Extract tokens: replace `#3b82f6` bằng `var(--color-primary)` — zero visual change, maximum reversibility.                                                           |
| 4   | Thiếu semantic token layer — component hardcode màu trực tiếp hoặc reference primitive tokens. Fix: tạo semantic tokens `--color-surface`, `--color-text-primary` → components reference semantic, không primitive → chỉ update 1 file theme.                                                                            |
| 5   | Design tokens = constants có tên có nghĩa cho design decisions. Primitive: `--spacing-4: 16px`. Semantic: `--spacing-card-padding: var(--spacing-4)` (tại sao 16px, không phải bao nhiêu). Component reference semantic → tách "giá trị" khỏi "ý nghĩa".                                                                 |

> 🎯 **Feynman Prompt:** Giải thích tại sao một codebase CSS lớn dần trở thành "mớ hỗn độn" — và ITCSS giải quyết điều đó như thế nào — dùng ví dụ tủ quần áo không có hệ thống vs tủ có ngăn kéo theo loại.
