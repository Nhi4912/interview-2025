...
- → Answer opens with: _"var is function-scoped, so it ignores block boundaries like if/for. let and const are block-scoped, creating a new binding for each block."_
...
**Chuỗi scope lồng nhau — tra cứu chỉ đi lên / Nested scope chain — variable lookup goes UP only:**
...
```text
  ...
  │  │  ┌─────────────────────────────────────────────────┐    │   │
  │  │  │  buildUrl() SCOPE                               │    │   │
  │  │  │  const path = '/users'  ← own variable          │    │   │
  │  │  │                                                 │    │   │
  │  │  │  baseUrl → not found here → look UP ▲           │    │   │
  │  │  │  path    → found here ✓                         │    │   │
  │  │  │  ENV     → not found here → look UP ▲ UP ▲      │    │   │
  │  │  └─────────────────────────────────────────────────┘    │   │
  │  │            ▲                             ▲               │   │
  │  │     baseUrl found ✓               (not here either)      │   │
  │  └──────────────────────────────────────────────────────────┘   │
  │                                             ▲                   │
  │                                      ENV found ✓                │
  └─────────────────────────────────────────────────────────────────┘
  ...
  KEY RULES:
  → Inner scopes look UP the chain: inner → outer → global → undefined
  → Outer scopes CANNOT see INTO inner scopes  (one-way street)
  → Chain FIXED at write time (lexical), not at call time
  → Shadowing: same name in inner scope hides the outer variable
```
...
- → Answer opens with: _"JavaScript uses lexical scope — the scope chain is determined at the time the function is written, not when it's called. So I need to look at where this function is defined in the source code."_
...
**Hình dung Hoisting và vùng TDZ / Hoisting & Temporal Dead Zone visualization:**

```text
  DECLARATION              BEFORE DECLARATION LINE            AFTER
  ─────────────────────────────────────────────────────────────────
  var x = 5
    compile: var x;   ← hoisted, initialized to undefined
    runtime:
      console.log(x)  →  undefined ✓  (no error — var is accessible)
      ...var x = 5...
      console.log(x)  →  5 ✓

  function foo() {}
    compile: entire body  ← fully hoisted
    runtime:
      foo()           →  works ✓  (callable before declaration line)

  let x / const x / class C
    compile: hoisted  ← but NOT initialized → enters TDZ
    runtime:
      ┌─── TOP OF BLOCK ───────────────────────────────────────────┐
      │                                                            │
      ...
      │   typeof x         →  ReferenceError ✗  (unlike var!)     │
      │                                                            │
      │   let x = 5;  ◄── TDZ ends at the declaration line        │
      │                                                            │
      │   ◄══════════ SAFE TO USE ══════════════════════►          │
      │                                                            │
      │   console.log(x)   →  5 ✓                                 │
  ...
  var fn = () => {}   (function expression)
    compile: var fn;  ← hoisted (value: undefined), body NOT hoisted
    runtime:
  ...
  ─────────────────────────────────────────────────────────────────
  QUICK SUMMARY:
  var       →  undefined    (usable, wrong value)
  function  →  full body    (callable immediately)
  let/const →  ❌ TDZ       (ReferenceError until declaration)
  var fn=() →  undefined    (TypeError if called before assignment)
```
...
- → Answer opens with: _"This depends on which type of declaration. var is hoisted and initialized to undefined, so accessing it before assignment gives undefined. let/const are hoisted but in TDZ, so accessing before declaration gives ReferenceError."_
