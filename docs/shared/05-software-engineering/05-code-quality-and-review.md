...
**Bài học:** Code review không phải "policing" — nó là knowledge sharing + quality gate. Reviewer phải biết *gì cần tìm*, không chỉ *nhìn xem code có đẹp không*.
...
- [ ] Can I list 5 things to check in every code review (not just style)?
- [ ] Can I explain technical debt with a concrete example — and how to "sell" it to management?
- [ ] Can I describe the Boy Scout Rule and when it can be harmful (over-refactoring)?
- [ ] Can I explain how to give feedback on code constructively (separate code from person)?
- 💬 **Feynman Prompt:** Giải thích "comment on code, not person" — đưa ra ví dụ cụ thể của một comment tồi và rewrite nó theo way that's both honest and constructive.
