...
### About Grab
- Grab vận hành superapp tại SEA (8 quốc gia), bài toán scale và latency có tính địa lý-thời gian thực.
- Backend văn hóa Go mạnh; thiết kế hệ thống gần production scenarios thay vì bài textbook.
- Tuyển dụng thường thực dụng: đánh giá khả năng giải quyết bài toán domain (ride, food, payments).

### Engineering Culture / Văn Hóa Kỹ Thuật
- Engineering culture data-driven, đo lường bằng latency/error/cost.
- Interview kỳ vọng ứng viên nói được trade-off theo production constraints.
- Collaboration trong môi trường đa quốc gia => communication rõ bằng tiếng Anh là bắt buộc.
...
- Study priority (official guide): **System Design > Go Depth > Medium-Hard DSA**.

## 2. Interview Process / Quy Trình Phỏng Vấn

### Typical Rounds / Các Vòng Thường Gặp
1. HackerRank OA (60-90m): 2-3 bài DSA medium-hard, đôi khi có SQL/debugging.
2. Technical Screen (45-60m): coding live + nền tảng CS.
3. Backend/Frontend Deep Dive (60m): đi sâu stack thật của role.
4. System Design 1 (60m): domain thực tế như ride-matching, payments.
5. System Design 2 hoặc Hiring Manager (45-60m): cho [Senior].
6. Behavioral/Culture (30-45m): stakeholder, ownership, incident stories.

### Timeline & Difficulty / Thời Gian và Độ Khó
- Thường 3-7 tuần từ OA tới quyết định.
- Khoảng cách giữa các vòng 3-7 ngày, nhanh nếu headcount urgent.
- Offer thường kèm discussion về level calibration và scope team.
...
- React performance ở realtime UI: map updates, websocket streams, rendering budgets.
- Micro-frontends, module boundaries, shared design systems.
- State synchronization giữa real-time data + cached API.
...
### Backend (Go)
- Go concurrency: goroutines/channels/interfaces/context cancellation là trọng tâm.
- gRPC, Kafka, Redis, sharding, CAP theorem applied cho payment consistency.
- Thiết kế chống lỗi: idempotency, retries, DLQ, exactly-once approximation.
...
- System design rất nặng: giả định traffic cỡ 1M+ RPS ở vùng peak.
- Bài toán hay gặp: ride matching, surge pricing, real-time location, payment processing.
- Candidate mạnh cần nêu region failover, partition strategy, SLA theo country.
...
- Scenario 1: **Ride dispatch** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 2: **Surge pricing** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 3: **Realtime location stream** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 4: **Payment reconciliation** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 5: **Merchant delivery ETA** — phân tích bottleneck, consistency requirement, và rollback strategy.

## 5. Behavioral / Cultural Fit

- Behavioral quan tâm execution under pressure và cross-market communication.
- Incident response stories có số liệu trước/sau sẽ rất mạnh.
- Ownership nghĩa là chốt kết quả, không chỉ mô tả task cá nhân.
...
1. [Junior] Design scenario: Ride dispatch (variant 1).
...
2. [Junior] Design scenario: Surge pricing (variant 2).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
3. [Junior] Design scenario: Realtime location stream (variant 3).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
4. [Junior] Design scenario: Payment reconciliation (variant 4).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
5. [Junior] Design scenario: Merchant delivery ETA (variant 5).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
6. [Junior] Design scenario: Ride dispatch (variant 6).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
7. [Junior] Design scenario: Surge pricing (variant 7).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
8. [Junior] Design scenario: Realtime location stream (variant 8).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
9. [Mid] Design scenario: Payment reconciliation (variant 9).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
10. [Mid] Design scenario: Merchant delivery ETA (variant 10).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
11. [Mid] Design scenario: Ride dispatch (variant 11).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
12. [Mid] Design scenario: Surge pricing (variant 12).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
13. [Mid] Design scenario: Realtime location stream (variant 13).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
14. [Mid] Design scenario: Payment reconciliation (variant 14).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
15. [Mid] Design scenario: Merchant delivery ETA (variant 15).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
16. [Mid] Design scenario: Ride dispatch (variant 16).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
17. [Mid] Design scenario: Surge pricing (variant 17).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
18. [Mid] Design scenario: Realtime location stream (variant 18).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
19. [Senior] Design scenario: Payment reconciliation (variant 19).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
20. [Senior] Design scenario: Merchant delivery ETA (variant 20).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
21. [Senior] Design scenario: Ride dispatch (variant 21).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
22. [Senior] Design scenario: Surge pricing (variant 22).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
23. [Senior] Design scenario: Realtime location stream (variant 23).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
24. [Senior] Design scenario: Payment reconciliation (variant 24).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
25. [Senior] Design scenario: Merchant delivery ETA (variant 25).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
