# Employment Hero Interview Guide — Hướng Dẫn Phỏng Vấn Employment Hero
...
- Employment Hero có quy trình async-first, phù hợp môi trường distributed teams.
- Vietnam engineering hub hoạt động mạnh; phần lớn vòng technical dùng tiếng Anh.
- Domain HR/payroll ảnh hưởng trực tiếp nội dung câu hỏi và case study.

### Engineering Culture / Văn Hóa Kỹ Thuật
- Giá trị “The EH Way” thường được kiểm tra kỹ: bold/ambitious, invent, own it.
- Communication rõ ràng bằng written/video format là năng lực bắt buộc.
- Xu hướng AI-first tăng dần: biết dùng AI tools đúng cách là điểm cộng thay vì cấm tuyệt đối.
...
- Study priority (official guide): **Values Fit > Product Domain > Practical Coding**.

## 2. Interview Process / Quy Trình Phỏng Vấn

### Typical Rounds / Các Vòng Thường Gặp
1. Async Recruiter Screen (video hoặc email): xác nhận motivation/timeline.
2. Loom Video Screen (15-30m): self-intro + technical walkthrough ngắn.
3. Practical Take-home (Low-Medium, 1-3 ngày): feature nhỏ có acceptance criteria rõ.
4. Technical Interview (45-60m): review solution, trade-offs, production readiness.
5. Values Interview (30-45m): kiểm tra “The EH Way” trong tình huống thật.

### Timeline & Difficulty / Thời Gian và Độ Khó
- Quy trình thường 2-5 tuần, phụ thuộc vòng async scheduling.
- Feedback thường theo batch, nên có thể chờ 3-7 ngày mỗi mốc.
- Ứng viên proactive follow-up bằng văn phong chuyên nghiệp thường được đánh giá cao.
...
- React practical feature delivery: forms, validation, UX clarity, testability.
- Kỳ vọng hiểu product metrics cho HR flows: completion rate, error rate, task latency.
- Đánh giá khả năng viết code maintainable hơn tối ưu thuật toán cực đoan.
...
### Backend (Go)
- Stack chính tổ chức: Ruby/Rails + React, .NET secondary, Go đang tăng dần.
- BE candidates nên chứng minh API design, payroll-safe calculations, audit logs.
- Khả năng làm việc liên stack và đọc codebase mixed-language là điểm cộng.
...
- System design vừa phải nhưng thiên product correctness (HR/payroll compliance).
- Nên nói rõ data privacy, role-based permissions, change history.
- Trade-off dễ gặp: tốc độ ship feature vs kiểm soát lỗi nghiệp vụ payroll.
...
- Scenario 1: **Payroll runs** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 2: **Leave management** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 3: **HRIS workflows** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 4: **Employee self-service** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 5: **Compliance reporting** — phân tích bottleneck, consistency requirement, và rollback strategy.

## 5. Behavioral / Cultural Fit

- Values fit thường có trọng số cao hơn thuật toán.
- Nên chuẩn bị ví dụ ownership từ discovery tới post-release metrics.
- Interviewers đánh giá cách bạn phản biện và đề xuất cải tiến process.
...
1. [Junior] Design scenario: Payroll runs (variant 1).
...
2. [Junior] Design scenario: Leave management (variant 2).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
3. [Junior] Design scenario: HRIS workflows (variant 3).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
4. [Junior] Design scenario: Employee self-service (variant 4).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
5. [Junior] Design scenario: Compliance reporting (variant 5).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
6. [Junior] Design scenario: Payroll runs (variant 6).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
7. [Junior] Design scenario: Leave management (variant 7).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
8. [Junior] Design scenario: HRIS workflows (variant 8).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
9. [Mid] Design scenario: Employee self-service (variant 9).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
10. [Mid] Design scenario: Compliance reporting (variant 10).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
11. [Mid] Design scenario: Payroll runs (variant 11).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
12. [Mid] Design scenario: Leave management (variant 12).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
13. [Mid] Design scenario: HRIS workflows (variant 13).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
14. [Mid] Design scenario: Employee self-service (variant 14).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
15. [Mid] Design scenario: Compliance reporting (variant 15).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
16. [Mid] Design scenario: Payroll runs (variant 16).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
17. [Mid] Design scenario: Leave management (variant 17).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
18. [Mid] Design scenario: HRIS workflows (variant 18).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
19. [Senior] Design scenario: Employee self-service (variant 19).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
20. [Senior] Design scenario: Compliance reporting (variant 20).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
21. [Senior] Design scenario: Payroll runs (variant 21).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
22. [Senior] Design scenario: Leave management (variant 22).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
23. [Senior] Design scenario: HRIS workflows (variant 23).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
24. [Senior] Design scenario: Employee self-service (variant 24).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
25. [Senior] Design scenario: Compliance reporting (variant 25).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
