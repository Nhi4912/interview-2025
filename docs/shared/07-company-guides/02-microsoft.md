...
### About Microsoft
- Microsoft có quy trình tuyển dụng đa team, thiên về impact trong domain cụ thể thay vì chỉ trick algorithms.
- Ứng viên có thể phỏng vấn song song nhiều team cùng lúc, tăng khả năng match headcount.
- Với cloud/product teams, tiêu chí compliance, security, và enterprise reliability rất nặng.

### Engineering Culture / Văn Hóa Kỹ Thuật
- Culture đề cao growth mindset, collaboration xuyên chức năng, và customer obsession cho enterprise.
- Interviewers thường chấm cả technical judgment, communication, và văn hóa hợp tác.
- Một số báo cáo cộng đồng cho thấy pure coding chỉ là một phần (khoảng 25%) trong rubric tổng.
...
- Study priority (official guide): **Domain Knowledge > Medium DSA > System Design**.

## 2. Interview Process / Quy Trình Phỏng Vấn

### Typical Rounds / Các Vòng Thường Gặp
1. Recruiter + HM Intro (30-45m): xác định level/team scope.
2. Coding Screen 1 (45-60m): LeetCode medium, Binary Trees xuất hiện rất thường xuyên.
3. Coding Screen 2 (45m): data structures + clean implementation.
4. Domain-specific Round (45-60m, đặc trưng Microsoft): problem trong bối cảnh sản phẩm/team.
5. System Design (45-60m): thường gắn Azure/compliance/data governance.
6. Behavioral + Collaboration (30-45m): cross-team, disagreement handling, ownership.

### Timeline & Difficulty / Thời Gian và Độ Khó
- Quy trình thường 3-8 tuần; team matching nội bộ có thể song song trong khi interview tiếp diễn.
- Feedback thường quay lại trong 2-7 ngày sau từng cụm vòng.
- Offer stage có thể phụ thuộc approval cycle và headcount refresh.
...
- React component architecture, state management, performance profiling.
- Domain round FE: accessibility, enterprise dashboards, long-lived state, reliability UX.
- Kỳ vọng TypeScript nghiêm túc, component contracts rõ, test strategy rõ.
...
### Backend (Go)
- API design, distributed service boundaries, Azure-native patterns (queues, pub/sub, identity).
- Compliance-heavy thinking: GDPR data lifecycle, audit log immutability, retention policy.
- Data pipeline reliability: schema evolution, replay safety, monitoring.
...
- System design nhấn mạnh governance và vận hành enterprise-scale hơn là whiteboard fantasy scale.
- Phải đề cập authN/authZ, tenant isolation, auditability, incident process.
- Trade-off cost trên Azure và operability thường là điểm phân tách strong candidate.
...
- Scenario 1: **Azure compliance workloads** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 2: **Enterprise reporting** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 3: **Identity platform** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 4: **Audit logging pipelines** — phân tích bottleneck, consistency requirement, và rollback strategy.
- Scenario 5: **Multi-tenant SaaS** — phân tích bottleneck, consistency requirement, và rollback strategy.

## 5. Behavioral / Cultural Fit

- Behavioral đánh giá cao teamwork, mentoring, influence không authority.
- Câu hỏi conflict resolution và stakeholder communication xuất hiện thường xuyên.
- Nên chuẩn bị ví dụ liên quan ship feature end-to-end trong môi trường đa nhóm.
...
1. [Junior] Design scenario: Azure compliance workloads (variant 1).
...
2. [Junior] Design scenario: Enterprise reporting (variant 2).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
3. [Junior] Design scenario: Identity platform (variant 3).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
4. [Junior] Design scenario: Audit logging pipelines (variant 4).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
5. [Junior] Design scenario: Multi-tenant SaaS (variant 5).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
6. [Junior] Design scenario: Azure compliance workloads (variant 6).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
7. [Junior] Design scenario: Enterprise reporting (variant 7).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
8. [Junior] Design scenario: Identity platform (variant 8).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
9. [Mid] Design scenario: Audit logging pipelines (variant 9).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
10. [Mid] Design scenario: Multi-tenant SaaS (variant 10).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
11. [Mid] Design scenario: Azure compliance workloads (variant 11).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
12. [Mid] Design scenario: Enterprise reporting (variant 12).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
13. [Mid] Design scenario: Identity platform (variant 13).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
14. [Mid] Design scenario: Audit logging pipelines (variant 14).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
15. [Mid] Design scenario: Multi-tenant SaaS (variant 15).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
16. [Mid] Design scenario: Azure compliance workloads (variant 16).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
17. [Mid] Design scenario: Enterprise reporting (variant 17).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
18. [Mid] Design scenario: Identity platform (variant 18).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
19. [Senior] Design scenario: Audit logging pipelines (variant 19).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
20. [Senior] Design scenario: Multi-tenant SaaS (variant 20).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
21. [Senior] Design scenario: Azure compliance workloads (variant 21).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
22. [Senior] Design scenario: Enterprise reporting (variant 22).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
23. [Senior] Design scenario: Identity platform (variant 23).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
24. [Senior] Design scenario: Audit logging pipelines (variant 24).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
25. [Senior] Design scenario: Multi-tenant SaaS (variant 25).
   - VN: Xác định requirement chức năng/phi chức năng, sau đó chốt SLO và capacity assumptions.
   - VN: Mô tả API contracts, storage strategy, caching, và observability tối thiểu.
   - VN: Nêu ít nhất 2 trade-off thực tế + 1 kế hoạch rollout an toàn.
